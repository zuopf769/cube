...
UOrderApp.pages.AddressListPageController.prototype.loadData = function (val) {
    var self = this;
    //加载地址列表
    cb.rest.getJSON('ma/Agents/getAgentShipToAddresses', function (data) {
        ...
        if (val.iReceiveId) {
            for (var i = 0; i < context.addressLists.length; i++) {
                if (context.addressLists[i].id == val.iReceiveId)
                    context.addressLists[i].isChecked = true;
            }
        }
        ...
        if (context.selectModel) {
            $$('#addressContainer').find('li').on('click', function () {
                if ($$('div[data-page="addrListPage"] .btn-addressManage').attr('data-Type') == 'edit') {
                    ...
                    backData = {
                        iReceiveId: backData[0].id,
                        cReceiver: backData[0].cReceiver,
                        cReceiveMobile: backData[0].cMobile,
                        cReceiveAddress: backData[0].cDistrct + backData[0].cAddress,
                        cReceiveZipCode: backData[0].cZipCode
                    };
                    var pageVewList = $$(myApp.mainView.pagesContainer).find('.page');
                    $$(myApp.mainView.pagesContainer).find('.page')[pageVewList.length - 1].f7PageData.query = { addressData: backData };
                    myApp.mainView.router.back({
                        query: { addressData: backData }
                    });
                }
            });
        }
