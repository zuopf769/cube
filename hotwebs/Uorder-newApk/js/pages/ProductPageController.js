...
UOrderApp.pages.ProductPageController.prototype.preprocess = function(content, pid, container, isList) {
	var context = {
		pid: pid
	};
	var self = this;
	...
	cb.rest.getJSON('ma/Products/getProduct', params, function(data) {
		...
		context.product = data.data.product;
		// 拼接图片URL
		...
		self.pageInit(context.product, isList);
	});
};
UOrderApp.pages.ProductPageController.prototype.pageInit = function(product, isList) {
	var self = this;
	cb.reKeyboard($$("#txtProSpecsCount"), $$("div[data-page=protDetailPage] .p-close"));
	var reloadSpec = function() {
		...
		cb.rest.getJSON('ma/Products/getSkus', params, function(skudata) {
			...
			var stepHtml = '<dl><dt>' + (skudata.data[0].iMinOrderQuantity?skudata.data[0].iMinOrderQuantity:"大于0") + '件 起订</dt><dd><em>￥</em>' + skudata.data[0].fSalePrice.toFixed(2) + '</dd></dl>';
	...
	var getDiscountPrice = function() {
		...
		cb.rest.getJSON('ma/Products/getDiscountPrice?skuId=' + self.sku.id + "&count=" + count, function(data) {
			...
			self.priceAount = upTool.numMulti(count, data.data);
	...
	$$('.popup-bottom .btn-update-sku').on('click', function(e) {
		...
		cb.rest.postData('mc/ShoppingCarts/add', orderJsonData, function(data) {
			...
			if (dataObj.code == 200 && dataObj.data) {
			    ...
			    //myApp.toast('加入购物车成功!', 'tips').show(true);
				if (isList) {
				    var proImg = $$("div[data-page=products] .smlProImg_f");
					proImg.find("img").attr("src", $$("div[data-page=protDetailPage] .popup-img img").attr("src").replace("lm_","s_"));
					var cartIco = $$("div[data-page=products] .animated");
					if (!self.proFiyTM) {
						proImg.show();
						proImg.addClass("profly_f");
						self.proFiyTM = setTimeout(function() {
							proImg.removeClass("profly_f");
							proImg.hide();
							cartIco.addClass("bounce");
							setTimeout(function() {
								cartIco.removeClass("bounce");
								self.proFiyTM = false;
							}, 1000);
						}, 1000);
					}
					...
					var proImg = $$("div[data-page=productDetail] .smlProImg");
					var cartIco = $$("div[data-page=productDetail] .animated");
					if (!self.proFiyTM) {
						proImg.show();
						proImg.addClass("profly");
						self.proFiyTM = setTimeout(function() {
							proImg.removeClass("profly");
							proImg.hide();
							cartIco.addClass("bounce");
							setTimeout(function() {
								cartIco.removeClass("bounce");
