...
UOrderApp.pages.NewPaybillController.prototype.pageInit = function (page) {
    ...
    //点击按钮保存
    $$('div[data-page="NewPaybill"] .button.btn-paySubmit').on('click', function () {
    ...
    $$('div[data-page="NewPaybill"] .upload.btn-uploadFile').on('click', function () {
        var buttons1 = [
       {
           text: '拍照',
           bold: true,
               ...
               if (window.plus && plus) {
                   ...
                   cmr.captureImage(function (p) {
                       plus.io.resolveLocalFileSystemURL(p, function (entry) {
                           ...
                           myApp.toast("读取拍照文件错误", 'error').show(true);
                       });
                   }, function (e) {
                       myApp.toast("拍照失败", 'error').show(true);
                   });
               }
           }
       },
...
UOrderApp.pages.NewPaybillController.prototype.InitData = function (val) {
    var bankHtml = function (data) {
        ...
        var singleBank = '<div class="item-title label">' + (data.type == 'Corp' ? '收款' : '汇款') + '</div>' +
                                    '<div class="item-media"><img src="' + cb.rest.appContext.serviceUrl + data.cPhoneImage + '" style="width:30px;height:30px;"/></div>' +
    ...
    $$('.popup.popup-select-bank').find('li').on('change', function (e) {
        ...
        if (id != '0' && bankData.length > 0) {
            ...
            newHTML = '<div class="item-title label">' + (type == 'Corp' ? '收款' : '汇款') + '</div>' +
                                    '<div class="item-media"><img src="' + cb.rest.appContext.serviceUrl + bankData.oBank.cPhoneImage + '" style="width:30px;height:30px;"/></div>' +
                                    '<div class="item-inner">' +
                                        '<div class="item-title-row">' +
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += '<li><label class="label-radio item-content">' + radio +
                    '<div class="media-item">' +
                        '<img src="' + cb.rest.appContext.serviceUrl + data[i].oBank.cPhoneImage + '" style="width:24px;height:24px;"/>' +
                    '</div>' +
                    '<div class="item-inner">' +
