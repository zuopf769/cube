...
UOrderApp.pages.BillController.prototype.formatData = function (val) {
    ...
    for (var i = 0; i < val.length; i++) {
        ...
        var newArray = val.filter(function (item) {
            return new Date(item.dPayCreateDate.replace(/-/g, "/").split('.')[0]) <= dPayCreateDate.endTime && new Date(item.dPayCreateDate.replace(/-/g, "/").split('.')[0]) >= dPayCreateDate.startTime;
        });
        for (var j = 0; j < newArray.length; j++) {
            ...
            var payDate = new Date(newArray[j].dPayCreateDate.replace(/-/g, "/").split('.')[0]);
