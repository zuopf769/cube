...
UOrderApp.pages.ClientInforController.prototype.pageInit = function (page) {
    ...
    $$('div[data-page="clientInfoPage"] .button.clientInfo-btn-submit').on('click', function () {
        ...
        if (formData.oSettlementWay.indexOf(']') > 0)
            saveData.iSettlementWayId = formData.oSettlementWay.subStr(1, 1);

        if (formData.oShippingChoice.indexOf(']') > 0)
            saveData.iShippingChoiceId = formData.oShippingChoice.subStr(1, 1);
    ...
    cb.rest.getJSON('ma/SettlementWays/getSettlementWays', function (data) {
        ...
        var oAgentCategoryPicker = myApp.picker({
            input: '#picker-clientInfo-oSettlementWay',
            toolbarCloseText: '关闭',
            value: ['[' + self.StorageData.oSettlementWay.id + ']' + self.StorageData.oSettlementWay.cName],
            cols: [
                {
                    textAlign: 'center',
                    values: arrayList
    ...
    cb.rest.getJSON('ma/ShippingChoices/getShippingChoices', function (data) {
        ...
        var oAgentCategoryPicker = myApp.picker({
            ...
            toolbarCloseText: '关闭',
            value: ['[' + self.StorageData.oShippingChoice.id + ']' + self.StorageData.oShippingChoice.cName],
            cols: [
                {
                    textAlign: 'center',
                    values: arrayList
    ...
    //加载地址信息
    $$('#picker-clientInfo-oAddress').parent('.item-input').on('click', function () {
