//var Refer_Extend = $.extend(ReferBase_Extend);
var Refer_Extend={
	...
	sortAction:function (viewModel) {

	},
	menuAction:function (viewModel) {
		debugger;
		viewModel.getCatalog().set("visible", true);
	},
    ...
    searchClick: function (viewModel, args) {
        ...
        if(grid){
			...
			grid.setDataSource(viewModel.getProxy(), 'load', options, function (data) {
	...
	catalogClick:function (viewModel, data) {
		...
		var tableInfo = { "code": data.pk, "pageSize": grid.getPageInfo().pageSize, "pageIndex": 1 };
		var options=this.getReadOptions(viewModel, null, tableInfo);
		
		viewModel.currentSelectNodeInfo = data;
		
		grid.setDataSource(viewModel.getProxy(), 'load', options, function (data) {
			self.setGridData(viewModel,data.table,true);
		}, function (data) {
		    return data.table;
		}, function (params, pageInfo) {
		    params.table = params.table || {};
	...
	queryTreeData: function (viewModel, nodeCode, pageSize, pageIndex, callBack) {
		...
		var options=this.getReadOptions(viewModel,treeInfo,null);
		if(options){
            var that = this;      
	...
	init_Extend:function (viewModel) {
		...
		var gridPageSize=viewModel.getTablePageSize();
		
		var treeInfo = { "code": "root", "pageSize": viewModel.getTreePageSize(), "pageIndex": 1 };
		//var treeInfo = null;
		var tableInfo = { "code": "", "pageSize": gridPageSize, "pageIndex": 1 };

		var self = this;
		var options=this.getReadOptions(viewModel, treeInfo, tableInfo);
		
		//必需保存grid的分页信息和服务请求的分页信息一致（服务中的分页pageIndex从1开始，grid中pageIndex从0开始）
		grid.setData({
		...
		grid.setDataSource(viewModel.getProxy(), 'load', options, function (data) {
			...
			//设置树和表的数据
			if (data.tree) {
				viewModel.getCatalog().setData({ pageSize: viewModel.getTablePageSize() });
				self.init_Tree(viewModel, data.tree);
				delete data.tree;
			}
			
			if (data.columnCode) {
				viewModel.set("columnCode", data.columnCode);
				delete data.columnCode;
			}
			viewModel.getSearch().set('placeholder', data.table.presetFilterTips);
			self.setGridData(viewModel,data.table,true);
		}, function (data) {
		    return data.table;
		}, function (params, pageInfo) {
